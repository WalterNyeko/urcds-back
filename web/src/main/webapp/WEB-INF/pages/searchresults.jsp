                        ...
                        <td>
                            <c:choose>
                                <c:when test="${crash.removed eq true}">
                                    <a href="<c:url value='/crashrestore'/>?id=${crash.id}" alt="Restore crash" onclick="return confirmDialog({message : 'Restore crash?', aLink : this});">
                                        <img src="<c:url value='/images/bt_Restore.gif'/>" alt="Restore" title="Restore" hspace="4">
                                    </a>
                                </c:when>
                                <c:otherwise>
                                    <a href="<c:url value='/crashview'/>?id=${crash.id}" alt="View crash">
                                        <img src="<c:url value='/images/bt_View.gif'/>" alt="View" title="View" hspace="4">
                                    </a>
                                    <c:if test="${crash.editable}">
                                        <a href="<c:url value='/crashform'/>?id=${crash.id}" alt="Edit crash">
                                            <img src="<c:url value='/images/bt_Edit.gif'/>" alt="Edit" title="Edit" hspace="4">
                                        </a>
                                    </c:if>
                                    <c:if test="${crash.removable}">
                                        <a href="<c:url value='/crashremove'/>?id=${crash.id}" alt="Remove crash" onclick="return confirmDialog({message : 'Remove crash?', aLink : this});">
